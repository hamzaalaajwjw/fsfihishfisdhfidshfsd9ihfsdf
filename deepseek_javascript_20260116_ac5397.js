...
<body>
  ...
  <div class="app">
    ...
    <main>
      ...
      <div id="articlesSection" class="content-section">
        ...
        <div id="loadMoreArticlesContainer" style="text-align: center; margin: 20px 0; display: none;">
          <button class="btn ghost" id="loadMoreArticlesBtn" onclick="loadMoreArticles()">
  ...
  <div class="modal-overlay" id="commentModal">
    <div class="modal" style="max-width: 500px;">
      ...
      <div class="modal-content">
        ...
        <div id="loadMoreCommentsContainer" style="text-align: center; margin: 16px 0; display: none;">
  ...
  import { getDatabase, ref, push, onValue, update, get, set, remove, increment, query, orderByChild, limitToLast, limitToFirst, startAfter, endBefore } from "https://www.gstatic.com/firebasejs/9.23.0/firebase-database.js";
  ...
  let lastArticleKey = null;
  let firstArticleKey = null;
  ...
  async function loadComments(articleId, loadMore = false) {
    ...
    if (lastCommentKey) {
      // تحميل التعليقات بعد المفتاح الأخير (للتحميل الإضافي)
      commentsQuery = query(
        commentsRef,
        orderByChild('created'),
        limitToLast(commentsPerPage + 1) // +1 للتحقق من وجود المزيد
      );
    } else {
      // تحميل أول 10 تعليقات
      commentsQuery = query(
    ...
    try {
      ...
      commentsSnap.forEach(commentSnap => {
        if (!loadMore || commentSnap.key !== lastCommentKey) {
          if (count < commentsPerPage) {
            comments.push({
              id: commentSnap.key,
              ...commentSnap.val()
            });
            newLastKey = commentSnap.key;
            count++;
          }
        }
      });
      
      // تحديد إذا كان هناك المزيد من التعليقات
      hasMoreComments = commentsSnap.numChildren() > commentsPerPage;
      ...
      comments.forEach(comment => {
        ...
        commentEl.innerHTML = `
          <div class="comment-header">
            <div class="comment-author">${comment.author ? 'مستخدم' : 'مجهول'}</div>
  ...
  window.loadMoreComments = () => {
    ...
    loadComments(currentArticleId, true);
  };
  ...
  async function loadArticles() {
    ...
    try {
      ...
      snap.forEach(articleSnap => {
        ...
        lastArticleKey = articleSnap.key;
        if (!firstArticleKey) firstArticleKey = articleSnap.key;
      });
      
      // تحقق إذا كان هناك المزيد من المقالات
      await checkIfMoreArticlesExist();
      
      // تحميل الإعجابات للمستخدم الحالي فقط
      if (currentUser) {
        try {
          await loadUserLikes();
        } catch (error) {
      ...
      // إظهار زر تحميل المزيد إذا كان هناك المزيد
  ...
  async function checkIfMoreArticlesExist() {
    try {
      const articlesRef = ref(db, 'articles');
      let checkQuery;
      ...
      const checkSnap = await get(checkQuery);
      hasMoreArticles = checkSnap.numChildren() > articlesPerPage;
    } catch (error) {
      console.error('Error checking for more articles:', error);
    ...
    try {
      ...
      if (currentFilter === 'popular') {
        ...
        articlesQuery = query(
          articlesRef,
          orderByChild('likesCount'),
          endBefore(lastArticleKey ? null : 0), // نحتاج إلى مزيد من المنطق هنا
          limitToLast(articlesPerPage)
        );
      } else {
        // الأحدث - حسب التاريخ
        articlesQuery = query(
          articlesRef,
          orderByChild('created'),
          endBefore(lastArticleKey ? allArticles[allArticles.length - 1].created : 0),
          limitToLast(articlesPerPage)
        );
      }
      ...
      snap.forEach(articleSnap => {
        // تجنب تكرار المقالات
        if (!allArticles.some(article => article.id === articleSnap.key)) {
          const articleData = {
            id: articleSnap.key,
            ...articleSnap.val()
          };
      ...
      lastArticleKey = newLastKey;
      
      // تحقق مرة أخرى إذا كان هناك المزيد من المقالات
      await checkIfMoreArticlesExist();
      
      // تحميل الإعجابات للمقالات الجديدة
      if (currentUser) {
        await loadUserLikesForArticles(newArticles);
      }
  ...
  async function renderArticle(article) {
    ...
    // إنشاء أزرار الإعجاب والتعليق (بدون دوائر)
    ...
    card.innerHTML = `
      ...
      <div class="article-meta">
        ...
        <span><i class="far fa-user"></i> ${article.author ? 'مستخدم' : 'مجهول'}</span>
  ...
  async function openMyArticles() {
    ...
    // هنا نحتاج لتحميل جميع مقالات المستخدم، لكن يمكن تحسينه لاحقاً
    const articlesRef = ref(db, 'articles');
    const articlesQuery = query(
      articlesRef,
      orderByChild('author'),
      startAfter(currentUser.uid),
      limitToFirst(100) // حد معقول
    );
