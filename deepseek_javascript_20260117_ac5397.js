  ...
  async function fetchArticles() {
    ...
    if (currentFilter === 'popular') {
      ...
      if (lastArticleTimestamp) {
        articlesQuery = query(
          ...
          endBefore(lastArticleTimestamp),
          limitToLast(articlesPerPage)
        );
      } else {
        articlesQuery = query(
    ...
    if (snap.exists()) {
      snap.forEach(articleSnap => {
        ...
        if (!lastArticleTimestamp || article.created < lastArticleTimestamp) {
          lastArticleTimestamp = article.created;
        }
      });
    }
    ...
    try {
      ...
      if (lastCommentTimestamp) {
        commentsQuery = query(
          commentsRef,
          orderByChild('created'),
          endBefore(lastCommentTimestamp),
